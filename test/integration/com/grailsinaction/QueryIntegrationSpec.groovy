...
class QueryIntegrationSpec extends IntegrationSpec {
	...
	void "simple property comparison"(){
		when: "Users are selected by simple password match"
	...
	void "Muitlple criteria"(){
		...
		users*.loginId == ["dollon", "frankie", "sara"]
	}
	
	void "Query on associtation"(){
		when: "The'following' collection is queried"
		def users = User.where{ following.loginId == "sara"}.list(sort: "loginId")
	...
	void "Query against a range value"(){
		...
		def users = User.where{ dateCreated in (now - 1)..now}.list(sort: "loginId", order: "desc")
